  ...
  async function fetchMoods() {
    ...
    moodsContainer.innerHTML = moods
      .map(
        (mood) => `
      <div>
        <a class="block p-3 rounded-lg bg-[rgba(255,255,255,0.1)] hover:bg-[rgba(255,255,255,0.2)]" href="/moods/${mood.slug}" data-navigo>${mood.name}</a>
      </div>
    `
      )
      .join("");
  ...
  async function fetchAlbums() {
    ...
    albumsContainer.innerHTML = albums
      ...
      <div class="flex flex-col gap-4 flex-shrink-0">
        <a href="/albums/${album.slug}" data-navigo>
        ...
        <div>
          <div>
            <a class="hover:underline" href="/albums/${
              album.slug
            }" data-navigo>${album.title}</a>
          </div>
